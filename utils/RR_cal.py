...
def benchmark(cfg,datasets,max_iter):
    c_flags={}
    c_errors={}
    ...
    result_dir=f'{cfg.output_cache_fn}/{wholesetname}/Eval_results_RR_{max_iter}iters'
    ...
    for scene,dataset in datasets.items():
        ...
        if cfg.score:
            if cfg.ir:
                pre_dir=f'{cfg.output_cache_fn}/{dataset.name}/score*ir_trans'
            else:
                pre_dir=f'{cfg.output_cache_fn}/{dataset.name}/score_trans'
        else:
            # pre_dir=f'{cfg.output_cache_fn}/{dataset.name}/ceiling_trans'
            # pre_dir=f'{cfg.output_cache_fn}/{dataset.name}/ir_trans'
        ...
        temp_precision, temp_recall,c_flag,c_error = evaluate_registration(n_fragments, est_traj, est_pairs, gt_pairs, gt_traj, gt_traj_cov,err2=cfg.RR_dist_th,nonconsecutive=nonconsecutive)
